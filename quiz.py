...
QUESTIONS_CSV_URL = (
    "https://raw.githubusercontent.com/agoesd/quizPBJ/main/quiz_questions.csv"
)
questions = load_questions(QUESTIONS_CSV_URL)
...
if st.session_state.get("quiz_started"):
    ...
    selected_option = st.radio(
        prompt,
        question["options"],
        key=f"options_{st.session_state['question_index']}",
    )
        ...
        if st.session_state["question_index"] >= num_questions:
            ...
            st.session_state["score"] = sum(
                ...
                for answer, question in zip(
                    ...
                    st.session_state.get("selected_questions", []),
                )
            )
